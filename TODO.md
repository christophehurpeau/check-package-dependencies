...
>
> - One API check may be split across multiple rules (e.g. the `min-range-*` family is two rules; the workspace-related API logic is bundled into one `consistent-workspace-dependencies` rule).
> - One rule may cover several API checks at once (e.g. `consistent-workspace-dependencies` covers both `checkDuplicateDependencies` cross-checks and `checkMonorepoDirectSubpackagePeerDependencies`).
> - A rule can be a strict subset of its API equivalent (e.g. `no-root-workspace-dependencies` only handles the workspace-root case of `checkNoDependencies`; the `require-exact-versions` rule has no equivalent to `internalExactVersionsIgnore`).
...
| ----------------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `checkExactVersions`                                  | `checkExactVersions`                                | `require-exact-versions` ✅                                                                    | `checkExactVersions.test.ts` ✅                                                                |
| `checkExactDevVersions`                               | `checkExactVersions` (devDeps only)                 | covered by `require-exact-versions` (partial) ⚠️                                               | covered by `checkExactVersions.test.ts` ⚠️                                                     |
...
| `checkNoDependencies`                                 | `checkNoDependencies`                               | `no-root-workspace-dependencies` ⚠️ (workspace root only, via `check-package-with-workspaces`) | `checkNoDependencies.test.ts` ✅                                                               |
| `checkDirectPeerDependencies`                         | `checkDirectPeerDependencies`                       | `require-direct-peer-dependencies` ✅                                                          | `checkDirectPeerDependencies.test.ts` ✅                                                       |
| `checkDirectDuplicateDependencies`                    | `checkDirectDuplicateDependencies`                  | `no-direct-duplicate-dependencies` ✅                                                          | ❌ no test file                                                                                |
| `checkResolutionsHasExplanation`                      | `checkResolutionsHasExplanation`                    | `require-resolutions-explanation` ✅                                                           | ❌ no test file                                                                                |
| `checkRecommended`                                    | meta-check (combines others)                        | N/A                                                                                            | ❌ no test file                                                                                |
| `checkIdenticalVersionsThanDependency`                | `checkIdenticalVersionsThanDependency`              | `require-identical-versions-as-dependency` ✅                                                  | `checkIdenticalVersionsThanDependency.test.ts` ✅                                              |
| `checkIdenticalVersionsThanDevDependencyOfDependency` | `checkIdenticalVersionsThanDependency` (on devDeps) | `require-identical-versions-as-dev-dependency-of-dependency` ✅                                | no dedicated unit test (core function is source-agnostic; ESLint rule has integration test) ⚠️ |
| `checkSatisfiesVersions`                              | `checkSatisfiesVersions`                            | `satisfies-versions` ✅                                                                        | `checkSatisfiesVersions.test.ts` ✅                                                            |
...
| `checkIdenticalVersions`                              | `checkIdenticalVersions`                            | `require-identical-versions` ✅                                                                | `checkIdenticalVersions.test.ts` ✅                                                            |
...
| ---------------------------------- | ---------------------------------- | ----------------------- | ------------------------------- |
| `checkExactVersions`               | `require-exact-versions`           | ✅                      | ✅ (`dependencies: false`)      |
| `checkResolutionsVersionsMatch`    | `resolutions-versions-match`       | ✅                      | ✅                              |
| `checkResolutionsHasExplanation`   | `require-resolutions-explanation`  | ✅                      | ✅                              |
| `checkDirectPeerDependencies`      | `require-direct-peer-dependencies` | ✅                      | ✅                              |
| `checkDirectDuplicateDependencies` | `no-direct-duplicate-dependencies` | ✅                      | ✅                              |
...
| ------------------------------- | -------------------------------- | ---------------- | ------------------------ |
| `checkNoDependencies()` on root | `no-root-workspace-dependencies` | ✅               | ✅                       |
...
Subpackage `checkRecommended` also passes `internalExactVersionsIgnore: [...workspace package names]` so workspace-internal package names are skipped by `checkExactVersions`. **The `require-exact-versions` ESLint rule does not currently replicate this**: it has no option to ignore workspace package names. ⚠️ divergence — if a subpackage depends on another workspace package with a range like `^1.0.0`, the programmatic flow allows it while the ESLint rule will report it.

**Phase 4 — monorepo cross-checks**

| Programmatic call                                                                                                                  | ESLint rule                                   | In `recommended` | In `recommended-library` |
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- | ---------------- | ------------------------ |
| `checkDuplicateDependencies` (root↔sub, sub↔sub for `dependencies`/`devDependencies`/`peerDependencies`)                           | `consistent-workspace-dependencies` (partial) | ✅               | ✅                       |
| `checkMonorepoDirectSubpackagePeerDependencies` → `checkSatisfiesPeerDependency` (sub's deps' peer deps satisfied by root devDeps) | `consistent-workspace-dependencies` (partial) | ✅               | ✅                       |

`consistent-workspace-dependencies` only fires on a `package.json` that has a `workspaces` field, so the cross-checks always run against the monorepo root.
...
- Every check in the programmatic `CheckPackageWithWorkspacesApi.checkRecommended` has a corresponding ESLint rule. The monorepo cross-checks are bundled into the single `consistent-workspace-dependencies` rule.
- ⚠️ **One known divergence**: the programmatic workspaces flow passes `internalExactVersionsIgnore` so workspace-internal package names skip exact-version checks; the `require-exact-versions` ESLint rule has no equivalent option yet.
- The three opt-in rules (`satisfies-versions`, `satisfies-versions-between-dependencies`, `satisfies-versions-from-dependencies`) are not part of any `checkRecommended` flow.
...
| ---------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------ |
| Root has no `dependencies`                                 | `checkNoDependencies` (root only)                                                | `no-root-workspace-dependencies` ✅              |
| Monorepo direct duplicate dependencies (root↔sub, sub↔sub) | `checkDuplicateDependencies` (internal)                                          | `consistent-workspace-dependencies` ✅ (partial) |
| Monorepo subpackage peer dependencies                      | `checkMonorepoDirectSubpackagePeerDependencies` → `checkSatisfiesPeerDependency` | `consistent-workspace-dependencies` ✅ (partial) |

## ESLint rule test coverage

| ESLint rule                                                  | Test file                                                                  |
| ------------------------------------------------------------ | -------------------------------------------------------------------------- |
| `require-exact-versions`                                     | `require-exact-versions.ruletester.test.ts` ✅                             |
...
| `require-direct-peer-dependencies`                           | `require-direct-peer-dependencies.test.ts` ✅                              |
| `no-direct-duplicate-dependencies`                           | `no-direct-duplicate-dependencies.test.ts` ✅                              |
| `resolutions-versions-match`                                 | `resolutions-versions-match.ruletester.test.ts` ✅                         |
| `require-resolutions-explanation`                            | `require-resolutions-explanation.ruletester.test.ts` ✅                    |
...
| `satisfies-versions`                                         | `satisfies-versions.ruletester.test.ts` ✅                                 |
| `no-root-workspace-dependencies`                             | `no-root-workspace-dependencies.ruletester.test.ts` ✅                     |
| `require-identical-versions`                                 | `require-identical-versions.test.ts` ✅                                    |
| `require-identical-versions-as-dependency`                   | `require-identical-versions-as-dependency.test.ts` ✅                      |
| `require-identical-versions-as-dev-dependency-of-dependency` | `require-identical-versions-as-dev-dependency-of-dependency.test.ts` ✅    |
...
| `consistent-workspace-dependencies`                          | `consistent-workspace-dependencies.test.ts` ✅                             |

## Checks not yet migrated to ESLint (summary)

- `checkNoDependencies` — partially migrated: `no-root-workspace-dependencies` covers the workspace-root case only; no ESLint rule yet for the standalone `checkNoDependencies(type)` use case
