...
describe("reportNotWarnedForMapping", () => {
    ...
    test("it report when warn not empty as array", () => {
        ...
        expect(reportError).toHaveBeenLastCalledWith('Invalid config in "test" for "*"', 'no warning was raised for "dep1"');
    });
    ...
    test("it report when warn not empty as record", () => {
        ...
        reportNotWarnedForMapping(reportError, onlyWarnsForMappingCheck);
        expect(reportError).toHaveBeenCalledTimes(1);
        expect(reportError).toHaveBeenLastCalledWith('Invalid config in "test" for "depKey"', 'no warning was raised for "dep1"');
    });
});
