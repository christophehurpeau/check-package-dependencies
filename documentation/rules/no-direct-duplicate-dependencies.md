...
While doing so, it also reports the checked dependencies declared in both `dependencies` and `devDependencies`, unless the package is a library (`isLibrary` setting) and the two values differ, which is the intended way to declare a range for consumers and an exact version for development.
