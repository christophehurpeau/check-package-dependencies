...
import type { PackageJson, ParsedPackageJson } from "../utils/packageTypes.ts";
import { parsePkgValue } from "../utils/pkgJsonUtils.ts";
import { checkResolutionVersionMatch } from "./checkResolutionsVersionsMatch.ts";

describe("checkResolutionVersionMatch", () => {
  const { mockReportError, messages } = createMockReportError();

  const checkAllResolutions = (pkgContent: PackageJson): ParsedPackageJson => {
    const parsedPkg = parsePkgValue(pkgContent);
    for (const resolutionValue of Object.values(parsedPkg.resolutions ?? {})) {
      if (resolutionValue) {
        checkResolutionVersionMatch(
          mockReportError,
          parsedPkg,
          resolutionValue,
        );
  ...
  it('should return no error when no "resolutions" is present', () => {
    checkAllResolutions({ name: "test", devDependencies: { test: "1.0.0" } });
    assertNoMessages(messages);
  });

  it('should return no error when "resolutions" has a dependency not in another dependency type', () => {
    checkAllResolutions({ name: "test", resolutions: { test: "1.0.0" } });
    assertNoMessages(messages);
  });

  it('should return no error when "resolutions" has matching dependencies', () => {
    checkAllResolutions({
      name: "test",
      resolutions: {
        test1: "1.0.0",
  ...
  it("should return an error for each dependency not matching", () => {
    checkAllResolutions({
      name: "test",
      resolutions: {
        test1: "1.0.0",
