...
describe("require-identical-versions", () => {
  it("should report when versions are not identical", async () => {
    ...
    const eslint = new ESLint({
      ...
      overrideConfig: {
        rules: {
          "check-package-dependencies-test/require-identical-versions": [
            "error",
            {
              devDependencies: {
                react: ["react-dom"],
    ...
    deepEqual(messages, [
      {
        ruleId: "check-package-dependencies-test/require-identical-versions",
        severity: 2,
        column: 5,
        endColumn: 25,
        endLine: 7,
